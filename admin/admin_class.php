...
Class Action {
	...
	function save_menu(){
		extract($_POST);
		$data = " name = '$name' ";
		$data .= ", price = '$price' "; //  name = 'jhh' , price = '999'
		$data .= ", category_id = '$category_id' "; //  name = 'jhh' , price = '999', category_id = '1'
		$data .= ", description = '$description' "; //  name = 'jhh' , price = '999', category_id = '1', description = 'abcd'
		if(isset($status) && $status  == 'on')
			$data .= ", status = 1 "; //  name = 'jhh' , price = '999', category_id = '1', description = 'abcd', status = 1
		else
			$data .= ", status = 0 ";		
		if($_FILES['img']['tmp_name'] != '') //for image
		{
			$fname = strtotime(date('y-m-d H:i')).'_'.$_FILES['img']['name'];
			$move = move_uploaded_file($_FILES['img']['tmp_name'],'../assets/img/'. $fname); //where to save image, image path
			$data .= ", img_path = '$fname' "; //  name = 'jhh' , price = '999', category_id = '1', description = 'abcd', status = 1, img_path = '12334456_bg_img.jpg'
		}
		if(empty($id)){
			$save = $this->db->query("INSERT INTO product_list set ".$data); 
		}else{
			$save_updated = $this->db->query("UPDATE product_list set ".$data." where id=".$id);
			$save = 0;
		}
		if($save == 1)
			return 1;
	...
	function add_to_cart(){
		...
		if(isset($_SESSION['login_user_id'])){
			...
			//$data =  "product_id = 1 , qty = 2 , user_id = 2"
			$is_cart = $this->db->query("SELECT id FROM cart WHERE product_id = $pid AND user_id = '".$_SESSION['login_user_id']."'"); //same product
			// print_r($is_cart);
			// return 0;
			$cart_rows = $is_cart->num_rows;	
			// print_r($cart_rows);
			// return 0;					
			if($cart_rows != 0)
			{
				$cart_id_row = $is_cart->fetch_assoc();
				$cart_id = $cart_id_row['id'];
				// print_r($cart_id);
				// return 0;
				$get_qty = $this->db->query("SELECT qty FROM cart WHERE product_id = $pid AND user_id = '".$_SESSION['login_user_id']."'");
				$qty_row = $get_qty->fetch_assoc();
				$updated_qty = $qty_row['qty']+$qty;
				// print_r($updated_qty);
				// return 0; 
				$save =  $this->db->query("UPDATE cart set user_id = '".$_SESSION['login_user_id']."', product_id = ".$pid.", qty = ".$updated_qty." where id = ".$cart_id);
			}
			else{
				$save = $this->db->query("INSERT INTO cart set ".$data); //return true
			}
